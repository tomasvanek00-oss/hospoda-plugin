...
require_once __DIR__ . '/includes/trait-hospoda-theme-settings.php';
require_once __DIR__ . '/includes/trait-hospoda-orders-core.php';

class Hospoda_Plugin {
    use Hospoda_Theme_Settings_Trait;
    use Hospoda_Orders_Core_Trait;
    ...
    private function inline_css_tag(){
        ...
        $css .= '.hsp-root .hsp-toggle{display:inline-block;padding:10px 16px;border:1px solid '.$week['toggle_border'].';border-radius:10px;background:'.$week['toggle_bg'].';color:'.$week['toggle_text'].';font-weight:700;letter-spacing:.2px;cursor:pointer;box-shadow:0 1px 2px rgba(0,0,0,.06)}';
        $css .= '.hsp-root .hsp-toggle:hover{background:'.$week['toggle_bg'].';border-color:'.$week['toggle_border'].';opacity:.92}';
        $css .= '.hsp-root .hsp-toggle:focus{outline:2px solid '.$week['toggle_focus'].';outline-offset:2px}';
        $css .= '.hsp-root .hsp-toggle[aria-expanded="true"]{background:'.$week['toggle_active_bg'].';border-color:'.$week['toggle_active_bg'].';color:'.$week['toggle_active_text'].'}';
        ...
        $css .= '.hsp-root .hsp-variant-badge{display:none!important}';
        $css .= '.hsp-root.hsp-variant-modern{font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif!important;background:linear-gradient(180deg,#f7faff 0%,#ddeafe 100%)!important;padding:20px!important;border-radius:28px!important;border:1px solid #bfdbfe!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-week{gap:18px!important}';
        $css .= '@media (min-width:960px){.hsp-root.hsp-variant-modern .hsp-week{grid-template-columns:1.15fr .85fr!important}}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-week__nav{justify-content:flex-start!important;flex-wrap:wrap!important;background:#ffffff!important;border:1px solid #c7d2fe!important;border-radius:999px!important;padding:10px 14px!important;box-shadow:0 12px 28px rgba(37,99,235,.12)!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-nav__label{order:-1!important;width:100%!important;font-size:.92em!important;text-transform:uppercase!important;color:#1e3a8a!important;letter-spacing:.08em!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-day{border:0!important;background:linear-gradient(180deg,#ffffff,#f8fbff)!important;border-radius:22px!important;box-shadow:0 18px 36px rgba(30,64,175,.14)!important;overflow:hidden!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-day__heading{font-size:1.18em!important;letter-spacing:0!important;background:linear-gradient(135deg,#0f172a,#1d4ed8)!important;color:#f8fafc!important;border-bottom:0!important;padding:18px 20px!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-body{padding:10px 20px 16px!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-grid{grid-template-columns:1fr!important;grid-template-areas:"price" "title" "sides"!important;gap:8px!important;padding:14px 0!important;border-bottom:1px solid #dbeafe!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-title{font-weight:700!important;font-size:1.07em!important;color:#0f172a!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-sides{font-size:.93em!important;color:#475569!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-price{justify-self:start!important;text-align:left!important;display:inline-flex!important;align-items:center!important;background:#dbeafe!important;color:#1d4ed8!important;padding:4px 10px!important;border-radius:999px!important;font-weight:800!important;letter-spacing:.02em!important;width:max-content!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-menu-groups{gap:14px!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-menu-group{background:#eff6ff!important;border:1px solid #93c5fd!important;border-radius:20px!important;padding:18px 20px!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-menu-group__title{display:block!important;color:#1e40af!important;font-size:1.1em!important;letter-spacing:0!important;text-transform:none!important}';
        $css .= '.hsp-root.hsp-variant-modern .hsp-menu-group__price{display:inline-flex!important;margin:8px 0 0!important;background:#1d4ed8!important;color:#fff!important;padding:3px 10px!important;border-radius:999px!important}';
    ...
    public function __construct() {
        ...
        add_action('admin_post_hsp_order_export_delivery_pdf', [$this,'handle_order_export_delivery_pdf']);
        add_action('admin_post_hsp_order_export_kitchen_csv', [$this,'handle_order_export_kitchen_csv']);
        add_action('admin_post_hsp_order_export_kitchen_pdf', [$this,'handle_order_export_kitchen_pdf']);
        add_action('admin_post_hsp_order_export_pickup_csv', [$this,'handle_order_export_pickup_csv']);
        add_action('admin_post_hsp_order_export_pickup_pdf', [$this,'handle_order_export_pickup_pdf']);
  ...
  <p class="hs-week-add"><button type="button" class="button add-row-week" data-week-index="<?php echo esc_attr($index); ?>" data-group-key="__GROUP_KEY__">Přidat jídlo</button></p>
    ...
    public function render_branding_admin_page() {
        ...
        $week_color_fields = [
            ...
            'toggle_bg'          => ['label' => 'Tlačítko „Zobrazit celý týden“ – pozadí'],
            'toggle_text'        => ['label' => 'Tlačítko „Zobrazit celý týden“ – text'],
            'toggle_border'      => ['label' => 'Tlačítko „Zobrazit celý týden“ – rámeček'],
            'toggle_active_bg'   => ['label' => 'Tlačítko po rozbalení – pozadí'],
            'toggle_active_text' => ['label' => 'Tlačítko po rozbalení – text'],
            'toggle_focus'       => ['label' => 'Tlačítko – focus obrys'],
        ];
        $static_color_fields = [
...
JS;
    ...
    private function normalize_day_for_week(string $day, string $week_start): string {
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $day)) {
            return $week_start;
        }
        $day_ts = strtotime($day);
        $week_ts = strtotime($week_start);
        if ($day_ts === false || $week_ts === false) {
            return $week_start;
        }
        $week_end = strtotime('+6 days', $week_ts);
        if ($week_end === false || $day_ts < $week_ts || $day_ts > $week_end) {
            return $week_start;
        }
        return $day;
    }

    private function build_day_meal_summary(array $orders, string $day): array {
        $soups = [];
        $mains = [];
        foreach ($orders as $order) {
            ...
            foreach ($items as $item) {
                ...
                $qty = max(1, (int)($item['quantity'] ?? 1));
                $bucket = ((string)($item['type'] ?? 'main') === 'soup') ? 'soups' : 'mains';
                if ($bucket === 'soups') {
                    $soups[$title] = ($soups[$title] ?? 0) + $qty;
                } else {
                    $mains[$title] = ($mains[$title] ?? 0) + $qty;
                }
            }
        }
        arsort($soups);
        arsort($mains);
        return ['soups' => $soups, 'mains' => $mains];
    }
    ...
    public function render_orders_admin_page() {
        ...
        $context['mode'] = 'week';
        $week_start = isset($_GET['filter_week']) ? sanitize_text_field((string) wp_unslash($_GET['filter_week'])) : (string)($context['week_start'] ?? '');
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $week_start)) {
            $week_start = (string)($context['week_start'] ?? wp_date('Y-m-d'));
        }
        $week_ts = strtotime($week_start);
        if ($week_ts === false) {
            $week_ts = current_time('timestamp');
        }
        $dow = (int) wp_date('N', $week_ts);
        $monday_ts = strtotime('-' . ($dow - 1) . ' days', $week_ts);
        if ($monday_ts === false) {
        ...
        $context['week_start'] = wp_date('Y-m-d', $monday_ts);
        $context['start'] = $context['week_start'];
        ...
        if ($orders_tab === 'day') {
            echo '<p>';
            echo '<a class="button" href="' . esc_url($this->get_orders_export_url('hsp_order_export_delivery_csv', $context)) . '">Rozvoz CSV</a> ';
            echo '<a class="button" href="' . esc_url($this->get_orders_export_url('hsp_order_export_delivery_pdf', $context)) . '">Rozvoz PDF</a> ';
            echo '<a class="button" href="' . esc_url($this->get_orders_export_url('hsp_order_export_kitchen_csv', $context)) . '">Kuchyň CSV</a> ';
            echo '<a class="button" href="' . esc_url($this->get_orders_export_url('hsp_order_export_kitchen_pdf', $context)) . '">Kuchyň PDF</a> ';
            echo '<a class="button" href="' . esc_url($this->get_orders_export_url('hsp_order_export_pickup_csv', $context)) . '">Vyzvednutí CSV</a> ';
            echo '<a class="button" href="' . esc_url($this->get_orders_export_url('hsp_order_export_pickup_pdf', $context)) . '">Vyzvednutí PDF</a>';
            echo '</p>';
    ...
    private function get_day_export_context_from_request(): array {
        $day = isset($_GET['filter_day']) ? sanitize_text_field((string) wp_unslash($_GET['filter_day'])) : '';
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $day)) {
            wp_die('Neplatný den exportu.');
        }
    ...
    private function get_order_item_labels_for_day(int $order_id, string $day): array {
        $labels = [];
        $items = get_post_meta($order_id, 'items', true);
        if (!is_array($items)) {
            return $labels;
        ...
        foreach ($items as $item) {
            ...
            $labels[] = (string)($item['title'] ?? '') . ' x' . (int)($item['quantity'] ?? 1);
        }
        return array_values(array_filter($labels));
    }

    public function handle_order_export_delivery_pdf() {
        if (!current_user_can('edit_posts')) {
            wp_die();
        }
        check_admin_referer('hsp_order_exports');

        $context = $this->get_day_export_context_from_request();
        $orders = $this->query_orders_for_context($context, 500);
        ...
        foreach ($orders as $order) {
            ...
            $labels = $this->get_order_item_labels_for_day((int)$order->ID, (string)$context['day_date']);
            if (empty($labels)) {
                continue;
            }
            $count++;
        ...
        if (!headers_sent()) {
            nocache_headers();
            header('Content-Type: application/pdf');
            header('Content-Disposition: attachment; filename="rozvoz-' . $context['day_date'] . '.pdf"');
    ...
    public function handle_order_export_kitchen_pdf() {
        if (!current_user_can('edit_posts')) {
            wp_die();
        }
        check_admin_referer('hsp_order_exports');
        ...
        foreach ($orders as $order) {
            ...
            foreach ($items as $item) {
                ...
                $aggregate[$title] = ($aggregate[$title] ?? 0) + (int)($item['quantity'] ?? 1);
            }
        }
        arsort($aggregate);
        ...
        if (!headers_sent()) {
            nocache_headers();
            header('Content-Type: application/pdf');
            header('Content-Disposition: attachment; filename="kuchyn-' . $context['day_date'] . '.pdf"');
            header('Content-Length: ' . strlen($content));
        }
        echo $content;
        exit;
    ...
    public function handle_order_export_pickup_csv() {
        if (!current_user_can('edit_posts')) {
            wp_die();
        }
        check_admin_referer('hsp_order_exports');
        ...
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename=vyzvednuti-' . $context['day_date'] . '.csv');
        $out = fopen('php://output', 'w');
        fputcsv($out, ['Filtr', $context['label']]);
        fputcsv($out, ['Jméno', 'Jídlo']);
        foreach ($orders as $order) {
            $dtype = (string) get_post_meta($order->ID, 'delivery_type', true);
            if ($dtype !== 'pickup') {
                continue;
    ...
    public function handle_order_export_pickup_pdf() {
        if (!current_user_can('edit_posts')) {
            wp_die();
        }
        check_admin_referer('hsp_order_exports');
        ...
        if (!headers_sent()) {
            nocache_headers();
            header('Content-Type: application/pdf');
            header('Content-Disposition: attachment; filename="vyzvednuti-' . $context['day_date'] . '.pdf"');
            header('Content-Length: ' . strlen($content));
        }
        echo $content;
        exit;
